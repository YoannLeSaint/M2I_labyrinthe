...
class Maze:
    def __init__(self, _maze_folder, maze_name):
        ...
        self._maze = array(line)

        self._start = (0, 0)
        self._end = (shape(self._maze)[0]-1, len(line[0])-1)
        self.visited = zeros( self._end ).astype(bool)
        print(self.visited)
    ...
    def get_neighbors(self, x, y):
        ...
        if x < self._end[0]-1 and not self.visited[x+1][y] and self._maze[x+1][y] != "#":
            neighbors.append((x+1, y))
        if y > 0 and not self.visited[x][y-1] and self._maze[x][y-1] != "#":
            neighbors.append((x, y-1))
        if y < self._end[1]-1 and not self.visited[x][y+1] and self._maze[x][y+1] != "#":
            neighbors.append((x, y+1))
        return neighbors
        ...
        while stack:
            ...
            if x == end_x and y == end_y:
                ...
                for neighbor_x, neighbor_y in neighbors:
                    ...
                    self.path.append((neighbor_y, neighbor_x))
        return False

    def __str__(self):
        output = ""
        print(self.path)
        for x in range(self._end[0]+1):
            for y in range(self._end[1]+1):
                if (x, y) == self.path[0]:
                    output += "S"
